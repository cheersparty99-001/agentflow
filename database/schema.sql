...
-- Migration for existing databases that still have the old chat_id column
ALTER TABLE customer_bot_links ADD COLUMN IF NOT EXISTS telegram_chat_id BIGINT;
ALTER TABLE customer_bot_links ADD COLUMN IF NOT EXISTS account_id UUID REFERENCES accounts(id);